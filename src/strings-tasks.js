...
function removeLastOccurrences(str, value) {
  // console.log(str.lastIndexOf(value));
  if (str.lastIndexOf(value) >= 0) {
    const res = str.split('');
    res.splice(str.lastIndexOf(value), value.length).join('');
    return res.join('');
    // return str.replace(
    //   str.slice(str.lastIndexOf(value), str.lastIndexOf(value) + value.length),
    //   ''
    // );
  }
  return str;
...
function countVowels(str) {
  ...
  str.split('').forEach((item) => {
    if (array.includes(item.toLocaleLowerCase())) {
      counter += 1;
    }
  });
  return counter;
...
function reverseWords(str) {
  const res = [];
  str.split(' ').forEach((item) => res.push(item.split('').reverse().join('')));
  return res.join(' ');
}
...
function encodeToRot13(str) {
  ...
  str.split('').forEach((item) => {
    if (reg.test(item)) {
      res.push(rot13[alphabet.indexOf(item)]);
    } else {
      res.push(item);
    }
  });
